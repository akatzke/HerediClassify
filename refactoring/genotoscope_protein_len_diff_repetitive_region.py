...
def check_prot_len_change_in_repetitive_region(
    ...
    variant_interval = BedTool(
        create_bed_line(variant, gen_start, gen_end, ref_transcript.strand), from_string=True
    )[0]
...
def check_prot_len_change_in_repetitive_region_start_loss(variant: VariantInfo, ref_transcript: pyensembl.transcript.Transcript, alt_start_codon: list[int], path_rep_uniprot: pathlib.Path):
    """
    ...
    prot_len_in_repetitive_region = check_prot_len_change_in_repetitive_region(variant, gen_start, gen_end, ref_transcript, path_rep_uniprot)
    return prot_len_in_repetitive_region


def check_prot_len_change_in_repetitive_region_exon(variant: VariantInfo, ref_transcript: pyensembl.transcript.Transcript, NMD_affected_exons:list[dict], path_rep_uniprot: pathlib.Path):
    if NMD_affected_exons:
        ...
        for exon in NMD_affected_exons:
            ...
            is_exon_in_repetitive_region = check_prot_len_change_in_repetitive_region(variant, gen_start, gen_end, ref_transcript, path_rep_uniprot)
...
def create_bed_line(variant: VariantInfo, gen_start: int, gen_end: int, transcript_strand: str) -> str:
    """
