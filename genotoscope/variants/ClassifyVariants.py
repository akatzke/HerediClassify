from genotoscope.utils import (
    create_dir,
    is_gsvar_file_extension,
    update_assignment,
    ...
    deformat_str2dict,
    load_hugo_genes_df,
    deactivate_lower_strength_rules,
    parse_maxentscan_pred,
    parse_dbscsnv_pred,
    is_variant_pathogenic,
    is_variant_conserved,
    aggregate_patho_predictions,
    aggregate_splicing_predictions,
)
...
class ClassifyVariants:
    ...
    def __init__(
        ...
        data_path,
        gsvar_folder,
        genes_inheritance_file,
        thresholds_inheritance_file,
        clinvar_root,
        clinvar_version,
        clinvar_file,
        clinvar_stars_file,
        min_review_stars,
        beds_root,
        critical_prot_version,
        critical_prot_regions_file,
        critical_prot_regions_no_benign_file,
        gnomAD_version,
        clinical_exons_file,
        uniprot_root,
        uniprot_version,
        uniprot_domains_file,
        uniprot_repeat_file,
        pheno_annot_root,
        phenotype_relevant_exons_file,
        pm1_regions_file,
        hugo_genes_file,
        pheno_high_af_variants_file,
        temp_path,
        output_path,
        filter_hl_genes,
        min_pathogenicity,
        ...
        self.critical_prot_regions_file = join(
            critical_prot_path, critical_prot_regions_file
        )
        self.critical_prot_regions_no_benign_file = join(
            critical_prot_path, critical_prot_regions_no_benign_file
        )
        ...
        self.phenotype_exons_file = join(
            pheno_annot_path, phenotype_relevant_exons_file
        )
        ...
        self.digram2inheritance = {
            "AR": "autosomal recessive",
            "AD": "autosomal dominant",
            "XL": "X-linked",
            "MT": "mitochondrial",
        }
        self.inheritance2digram = {
        ...
        if exists(join(self.data_path, pheno_high_af_variants_file)):
            self.pheno_high_af_variants = vcf.Reader(
                filename=join(self.data_path, pheno_high_af_variants_file),
                compressed=True,
                encoding="utf-8",
            )
        else:
    ...
    def init_logger(self, logging_root, logger_name, logging_level):
        ...
        # create file handler
        fh = logging_root.FileHandler(
            join(
                self.out_path,
                "genotoscope_classify_"
    ...
    def prepare_annotation_files(self):
        ...
        [proteome_id, species_taxid, annot_type, ref_version, _] = (
            basename(self.uniprot_domains_file).strip(".bed").split("_")
        )
        repeats_no_domains_filename = (
            "_".join([proteome_id, species_taxid, "repeats_no_domains", ref_version])
            + ".bed"
        )
        if not exists(join(self.uniprot_path, repeats_no_domains_filename)):
            self.logger.info(
                "Preparing annotation bed file: Uniprot repeats with no overlapping domain"
            )
            uniprot_repeats = BedTool(self.uniprot_repeat_file)
            uniprot_domains = BedTool(self.uniprot_domains_file)
            repeats_no_domains = uniprot_repeats.subtract(
                uniprot_domains, s=True
            ).sort()
            repeats_no_domains.saveas(
                join(self.uniprot_path, repeats_no_domains_filename)
            )
    ...
    def load_clinvar_review_stars_df(self, clinvar_review_stars_file):
        ...
        """
        clinvar_stars_df = read_csv(
            join(self.clinvar_root, clinvar_review_stars_file), sep="\t", header=0
        )
        clinvar_stars_df = clinvar_stars_df.rename(
            lambda x: x.strip().replace(" ", "_"), axis="columns"
        )
        clinvar_stars_df["Review_status"] = clinvar_stars_df[
            "Review_status"
        ].str.strip()
        clinvar_stars_df["Number_of_gold_stars"] = clinvar_stars_df[
    ...
    def process_column_line(self, gsvar_file, is_single_file_analysis=False):
        ...
        with open(join(gsvar_path, gsvar_file), "rb") as gsvar_in, open(
            join(self.temp_path, updated_gsvar_file), "w"
        ) as gsvar_out:
        ...
        assert (
            num_changed_lines == 1
        ), "AssertionError: only the column line should be changed"  # assert that only one line has been changed
    ...
    def find_strictest_inheritance_mode(self, rule_row):
        ...
        """
        if (
            str(rule_row["threshold_direction"]) == "greater"
            or str(rule_row["threshold_direction"]) == "greater_than_or_equal"
        ):
            ...
            str(rule_row["threshold_direction"]) == "lower"
            or str(rule_row["threshold_direction"]) == "lower_than_or_equal"
        ):
            # rule threshold direction is MAF < or <=
    ...
    def load_genes_inheritance_df(self):
        ...
        """
        self.logger.info(
            "Load genes inheritance mode from {}".format(
                join(self.data_path, self.genes_inheritance_file)
            )
        )
        self.genes_inheritance_df = read_excel(
            join(self.data_path, self.genes_inheritance_file), index_col=0, header=0
        )
    ...
    def load_thresholds_inheritance_df(self):
        ...
        """
        self.logger.info(
            "Load rule thresholds per inheritance mode from {}".format(
                join(self.data_path, self.thresholds_inheritance_file)
            )
        )
        self.inheritance_thresholds_df = read_csv(
            join(self.data_path, self.thresholds_inheritance_file), index_col=0
        )

        self.inheritance_thresholds_df[
            "strictest_inheritance"
        ] = self.inheritance_thresholds_df.apply(
    ...
    def load_gsvar_df(self, gsvar_dir_parent, gsvar_file):
        ...
        if gsvar_df.shape[0] != 0:
            ...
            if str(gsvar_df.loc[0, "chr"])[0:3] != "chr":
                ...
                gsvar_df["chr"] = gsvar_df["chr"].apply(
                    lambda chr_num: "chr" + str(chr_num)
                )
            ...
            self.logger.info(
                "Loaded GSvar file {} contains 0 variant rows.".format(gsvar_file)
            )
            is_gsvar_empty = True
    ...
    def examine_allele_freq(self, variants):
        ...
        if not isnan(float(variants["gnomAD"])):
            ...
            if is_var2exclude(
                self.pheno_high_af_variants,
                VariantInfo(
                    self.current_sample_name,
                    str(variants["gene"]).split(",")[0],
        ...
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], af_comment
        )
        return variants
    ...
    def var_pos2string(variant_row):
        ...
        str
                variant position
        """
        return "variant position: " + " ".join(
            [variant_row["chr"], str(variant_row["start"]), str(variant_row["end"])]
        )
    ...
    def examine_classified_class(self, variants):
        ...
        if "classification" not in variants.index.values:
            ...
            ngsd_class = "NGSD: not classified category"
            ngsd_comment = (
                "NGSD: database does not contain classified category for variant"
            )
        elif float(variants["classification"]) < 3:
            ngsd_class = "NGSD: discard variant"
            ngsd_comment = "NGSD: classification <3"
        else:
            ngsd_class = "NGSD: already classified category={}".format(
    ...
    def examine_pp3_bp4(self, variants):
        ...
        """
        self.logger.debug(
            "Examine PP3 and BP4 rules for variant: {}".format(
        ...
        if len(exonic_types.intersection(set(current_variant_types))) > 0:
            ...
            for predictor in patho_predictions.keys():
                patho_predictions[predictor], _ = is_variant_pathogenic(
                    variants, predictor
                )
            ...
            if is_var_conserved == 1 and is_var_pathogenic == "pathogenic":
                ...
                PP3_comment = (
                    "PP3: "
                    + " aggregation of REVEL and CADD predicts pathogenic mutation and PhyloP predicts conserved site"
                )
                BP4_class = "BP4: False"
                BP4_comment = (
                    "BP4: "
                ...
                PP3_comment = (
                    "PP3: "
                    + " aggregation of REVEL and CADD predicts benign mutation and PhyloP predicts no conserved site"
                )
                BP4_class = "BP4: True"
                BP4_comment = (
                    "BP4: "
                ...
                if is_var_pathogenic == "pathogenic":
                    patho_comment = (
                        "aggregation of REVEL and CADD predicts pathogenic mutation"
                    )
                elif is_var_pathogenic == "benign":
                    patho_comment = (
                        "aggregation of REVEL and CADD predicts benign mutation"
                    )
                else:
                    patho_comment = (
                        "no available REVEL nor CADD predictions to support evidence"
                    )
                if is_var_conserved == 1:
                    ...
                    conserved_comment = (
                        "no available conservation prediction to support evidence"
                    )
                PP3_comment = " ".join(
                    ["PP3:", patho_comment, "and", conserved_comment]
                )
                BP4_comment = " ".join(
                    ["BP4:", patho_comment, "and", conserved_comment]
                )
            ...
            if (
                splicing_predictions["MaxEntScan"] is None
                and splicing_predictions["dbscSNV"] is None
            ):
                ...
                self.logger.debug(
                    "Variant is classified as: {}".format(is_var_pathogenic)
                )
                self.logger.debug("Is variant conserved?: {}".format(is_var_conserved))
                self.logger.debug(
                    "Does variant affect splicing?: {}".format(impacts_splicing)
                )

                if (
                    impacts_splicing
                    and is_var_conserved == 1
                    and is_var_pathogenic == "pathogenic"
                ):
                    # variant affects splicing and it's conserved
                    PP3_class = "PP3: True"
                    PP3_comment = (
                        "PP3: "
                        + "aggregation of MaxEntScan and dbscSNV predicts splicing impact, "
                        + "aggregation of REVEL and CADD predicts pathogenic impact"
                        + " and PhyloP predicts conserved site"
                    )
                    BP4_class = "BP4: False"
                    BP4_comment = (
                        "BP4: "
                    ...
                    not impacts_splicing
                    and is_var_conserved == 0
                    and is_var_pathogenic == "benign"
                ):
                    PP3_class = "PP3: False"
                    PP3_comment = (
                        "PP3: "
                        + "aggregation of MaxEntScan and dbscSNV predicts no splicing impact, "
                        + "aggregation of REVEL and CADD predicts begign impact"
                        + " and PhyloP predicts no conserved site"
                    )
                    BP4_class = "BP4: True"
                    BP4_comment = (
                        "BP4: "
                    ...
                    if is_var_pathogenic == "pathogenic":
                        ...
                        patho_comment = (
                            "aggregation of REVEL and CADD predicts benign impact"
                        )
                    else:
        ...
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], PP3_comment
        )
        # add the BP4 rule
        variants["ACMG_rules"] = update_assignment(variants["ACMG_rules"], BP4_class)
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], BP4_comment
        )
        return variants
    ...
    def examine_bp7(self, variants):
        ...
        """
        self.logger.debug(
            "Examine BP7 rule for variant: {}".format(
                ClassifyVariants.var_pos2string(variants)
            )
        )
        current_variant_types = extract_all_variant_types(str(variants["variant_type"]))
        ...
        if "synonymous" in current_variant_types:
            ...
            if (
                ...
                self.logger.debug(
                    "Variant is predicted as: {}".format(is_var_pathogenic)
                )
                self.logger.debug("Is variant conserved?: {}".format(is_var_conserved))
                self.logger.debug(
                    "Does variant affect splicing?: {}".format(impacts_splicing)
        ...
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], BP7_comment
        )
        return variants
    ...
    def examine_quality_parameters(self, variants):
        ...
        for parameter in variants["quality"].split(";"):
            if parameter.split("=")[1].isdigit():
                quality_parameters[parameter.split("=")[0]] = float(
                    parameter.split("=")[1]
                )

        # examine quality, depth and MQM
        if (
            "DP" in quality_parameters
            and "QUAL" in quality_parameters
            and "MQM" in quality_parameters
        ):
            if quality_parameters["DP"] < 15:
                self.logger.debug(
                    "Assign: re-sequencing, because DP=%f <15x",
                    quality_parameters["DP"],
                )
                self.logger.debug("=> " + ClassifyVariants.var_pos2string(variants))
                quality_class = "QUAL: re-sequencing"
                quality_comment = "QUAL: low depth"
            elif (
                quality_parameters["DP"] < 20
                or quality_parameters["QUAL"] < 250
                or quality_parameters["MQM"] < 60
            ):
        ...
        variants["ACMG_rules"] = update_assignment(
            variants["ACMG_rules"], quality_class
        )
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], quality_comment
        )
        return variants
    ...
    def extract_inheritance_modes(self, gene_inheritance_info):
        ...
        if (
            ...
            assert (
                gene_inheritance_info[preferred_column] == 1
            ), "AssertionError: value of preferred inheritance, {}, should be 1".format(
                preferred_column
            )
    ...
    def extract_inheritance_modes_acmg(
        self, gene_name, gene_inheritance_infos, hpo_term
    ):
        """
        ...
        if len(gene_inheritance_infos.shape) == 1:
            ...
            disease2inheritance[
                str(gene_inheritance_infos["Disease association"]).strip()
            ...
            for _, gene_inheritance_row in gene_inheritance_infos.iterrows():
                if (
                    str(gene_inheritance_row["Disease association"]).strip()
                    not in disease2inheritance
                ):
                    disease2inheritance[
                        str(gene_inheritance_row["Disease association"]).strip()
                    ...
                    disease2inheritance[
                        str(gene_inheritance_row["Disease association"]).strip()
                    ].append(str(gene_inheritance_row["Inheritance Pattern"]).strip())
        self.logger.debug(
            "Disease to inheritance modes: {}".format(disease2inheritance)
        )
        ...
        if hpo_term:
            if hpo_term in disease2inheritance:
                if len(disease2inheritance[hpo_term]) == 1:
                    ...
                    self.logger.debug(
                        "The gene contains a single known inheritance mode for input HPO term"
                    )
                    preferred_inheritance, alternative_inheritance = (
                        disease2inheritance[hpo_term][0],
                        None,
                    )
                else:
                    # gene contains more than one known inheritance modes for input HPO
                    self.logger.debug(
                        "The gene contains more than one known inheritance mode for input HPO term"
                    )
                    preferred_inheritance, alternative_inheritance = (
                        disease2inheritance[hpo_term][0],
                ...
                self.logger.debug(
                    "HPO term={} is not matching known disease associations of the gene={}".format(
                        hpo_term
                    )
                )
            ...
            if len(disease2inheritance) == 1:
                ...
                if len(known_inheritances) == 1:
                    ...
                    preferred_inheritance, alternative_inheritance = (
                        known_inheritances[0],
                        None,
                    )
                else:
                    # single disease association, with multiple inheritances
                    self.logger.debug(
                        "Single disease association with more than one known inheritances"
                    )
                    preferred_inheritance, alternative_inheritance = (
                        known_inheritances[0],
                        known_inheritances[1],
                    )
            else:
                ### ### ###
                ...
                if all_inheritances_equal:
                    self.logger.debug(
                        "All associated diseases contain equal inheritance modes"
                    )
                    if len(first_disease_inheritances) == 2:
                        preferred_inheritance, alternative_inheritance = (
                            first_disease_inheritances[0],
                    ...
                    preferred_inheritance, alternative_inheritance = None, None
        self.logger.debug(
            "Final preferred: {}, alternative: {}".format(
                preferred_inheritance, alternative_inheritance
            )
        )
    ...
    def is_threshold_rule_triggered(self, rule_name, inheritance_mode, pop_MAF):
        ...
        """
        self.logger.debug(
            "Examine if rule: {} is triggered, with pop MAF={}".format(
                rule_name, pop_MAF
            )
        )
        threshold_direction = str(
            self.inheritance_thresholds_df.loc[rule_name, "threshold_direction"]
        )
        ...
        if inheritance_mode == "unknown":
            ...
            inheritance_mode = str(
                self.inheritance_thresholds_df.loc[rule_name, "strictest_inheritance"]
            )
        if threshold_direction == "greater":
            if pop_MAF > float(
                self.inheritance_thresholds_df.loc[rule_name, inheritance_mode]
            ):
                return True
            else:
                return False
    ...
    def examine_autosomal_recessive_MAF(self, gnomAD_subpopulations, exclude_var):
        ...
        for rule_name in examined_rules.keys():
            for subpop, subpop_MAF in gnomAD_subpopulations.items():
                if self.is_threshold_rule_triggered(rule_name, "AR", subpop_MAF):
                    ...
                    examined_rules_comment[rule_name].append(
                        "{}: {} {} {}".format(
                            subpop,
                            subpop_MAF,
                            self.inheritance_thresholds_df.loc[
                                rule_name, "threshold_direction"
                            ].replace("_", " "),
                            self.inheritance_thresholds_df.loc[rule_name, "AR"],
                        )
                    )
        ...
        for rule_name, variant_class_presence in examined_rules.items():
            if (
                rule_name == "BA1"
                or rule_name == "BS1"
                or rule_name == "BS1_Supporting"
            ) and exclude_var:
                ACMG_rules.append(rule_name + ": False")
                ACMG_rules_comment.append(
                    rule_name
                    + ": variant in list of phenotype relevant and pathogenic variants with high AF"
                )
        ...
        (
            ACMG_rules_valid,
            ACMG_rules_valid_comment,
            examined_rules_valid,
        ) = deactivate_lower_strength_rules(
        ...
        if len(ACMG_rules_valid) > 0:
            return (
                "|".join(ACMG_rules_valid),
                "|".join(ACMG_rules_valid_comment),
                examined_rules_valid,
            )
                ...
                if self.is_threshold_rule_triggered(rule_name, "AD", subpop_MAF):
                    ...
                    examined_rules_comment[rule_name].append(
                        "{}: {} {} {}".format(
                            ...
                            self.inheritance_thresholds_df.loc[rule_name, "AD"],
                        )
                    )
        # b) aggregate MAF examination per sub-population
        ACMG_rules, ACMG_rules_comment = [], []
    ...
    def examine_unknown_inheritance_MAF(self, gnomAD_subpopulations, exclude_var):
        ...
        for rule_name, rule_row in self.inheritance_thresholds_df.iterrows():
            if not (isnan(float(rule_row["AR"])) and isnan(float(rule_row["AD"]))):
                ...
                self.logger.debug(
                    "Rule: {} contains unknown MAF thresholds for AR and AD inheritance modes".format(
                        rule_name
                    )
                )
    ...
    def extract_gene_inheritance_mode(self, gene_name, use_phenotype_inheritance):
        ...
        if gene_name in self.genes_inheritance_df.index:
            if use_phenotype_inheritance:
                return self.extract_inheritance_modes_acmg(
                    gene_name, self.genes_inheritance_df.loc[gene_name], hpo_term
                )
            else:
    ...
    def aggregate_preferred_alternative_rules_assignments(
        self,
        examined_rules_preferred,
        inheritance_preferred,
        examined_rules_alternative,
        ...
        ### ### ###
        self.logger.debug(
            "Aggregate assignments for preferred and alternative inheritance mode"
        )
        self.logger.debug(
            "Preferred: {}, mode: {}".format(
                examined_rules_preferred, inheritance_preferred
            )
        )
        self.logger.debug(
            "Alternative: {}, mode: {}".format(
        ...
        for rule_name in examined_rules_preferred:
            if rule_name in examined_rules_alternative:
                ...
                if (
                    sum(examined_rules_preferred[rule_name]) > 0
                    and sum(examined_rules_alternative[rule_name]) > 0
                ):
                    # variant passes threshold for both inheritance modes
                    merged_assignments[rule_name] = "True"
                elif (
                    sum(examined_rules_preferred[rule_name]) > 0
                    or sum(examined_rules_alternative[rule_name]) > 0
                ):
                    # variant passes threshold for only one inheritance mode
                    self.logger.debug(
                        "For rule: {}, strictest_inheritance= {}".format(
                            rule_name,
                            self.inheritance_thresholds_df.loc[
                                rule_name, "strictest_inheritance"
                            ],
                    ...
                    if (
                        ...
                        if sum(examined_rules_preferred[rule_name]) > 0:
                            ...
                            merging_flags.append(
                                rule_name
                                + ": triggered not for the strictest inheritance mode"
                            )
                    elif (
                        self.inheritance_thresholds_df.loc[
                            rule_name, "strictest_inheritance"
        ...
        aggregated_assignment = "||".join(
            [
                ...
                for rule_name, rule_assignment in merged_assignments.items()
            ]
        )
    ...
    def examine_inheritance_specific_MAF(self, variants):
        ...
        """
        self.logger.debug(
            "Examine inheritance-specific rule for: ".format(
                ClassifyVariants.var_pos2string(variants)
            )
        )
        ...
        ) = self.extract_gene_inheritance_mode(gene, use_phenotype_inheritances)
        ...
        self.logger.debug(
            "Resulted inheritances: preferred: {}, alternative: {}".format(
                preferred_inheritance, alternative_inheritance
            )
        )
        ### ### ###
        ...
        if "gnomAD_sub" in variants:
            ...
            if "," not in str(variants["gnomAD_sub"]):
                ...
                if isnan(float(variants["gnomAD"])):
                    ...
                    self.logger.debug(
                        "Not present in BOTH gnomAD subpopulations and general gnomAD, assume MAF=0"
                    )
                    comment.append(
                        "Not present in BOTH gnomAD subpopulations and general gnomAD, assume MAF=0"
                    )
                    ...
                    self.logger.debug(
                        "Not present in gnomAD, fall back to general gnomAD: {}".format(
                ...
                for subpop_index, subpop_MAF in enumerate(
                    str(variants["gnomAD_sub"]).split(",")
                ):
                    if subpop_MAF == "":
                ...
                gnomAD_subpopulations[
                    self.position2gnomAD_sub[max_MAF_subpop_index]
                ] = max_MAF_subpop
        else:
            ...
            if isnan(float(variants["gnomAD"])):
                ...
                gnomAD_subpopulations["general"] = float(variants["gnomAD"])
                self.logger.debug(
                    "gnomAD subpopulation column is not given, fall back to general gnomAD: {}".format(
                        gnomAD_subpopulations["general"]
                    )
                )
                comment.append(
        ...
        self.logger.debug(
            "Parsed gnomAD subpopulations: {}".format(gnomAD_subpopulations)
        )
        ...
        exclude_var_benign_criteria = is_var2exclude(
            self.pheno_high_af_variants, current_variant
        )
        self.logger.debug("Exclude variant: {}".format(exclude_var_benign_criteria))

        # b) examine preferred inheritance mode
        self.logger.debug(
            "Examine preferred inheritance mode: {}".format(preferred_inheritance)
        )
        if preferred_inheritance == "AD":  # autosomal dominant
            (
                ACMG_assignments_preferred,
                ACMG_assignment_comment_preferred,
                examined_rules_preferred,
            ) = self.examine_autosomal_dominant_MAF(
                gnomAD_subpopulations, exclude_var_benign_criteria
            )
        elif (
            preferred_inheritance == "AR" or preferred_inheritance == "XL"
        ):  # autosomal recessive or X-linked
            (
                ACMG_assignments_preferred,
                ACMG_assignment_comment_preferred,
                examined_rules_preferred,
            ) = self.examine_autosomal_recessive_MAF(
                gnomAD_subpopulations, exclude_var_benign_criteria
            )
            ...
            (
                ...
                ACMG_assignment_comment_preferred,
                examined_rules_preferred,
            ) = self.examine_unknown_inheritance_MAF(
                gnomAD_subpopulations, exclude_var_benign_criteria
            )
        ...
        if alternative_inheritance:
            ...
            if alternative_inheritance == "AD":  # autosomal dominant
                (
                    ACMG_assignments_alternative,
                    ACMG_assignment_comment_alternative,
                    examined_rules_alternative,
                ) = self.examine_autosomal_dominant_MAF(
                    gnomAD_subpopulations, exclude_var_benign_criteria
                )
            elif (
                alternative_inheritance == "AR" or alternative_inheritance == "XL"
            ):  # autosomal recessive or X-linked
                (
                    ACMG_assignments_alternative,
                    ACMG_assignment_comment_alternative,
        ...
        variants["ACMG_rules"] = update_assignment(
            variants["ACMG_rules"], ACMG_assignments
        )
        variants["ACMG_rules_comment"] = update_assignment(
            variants["ACMG_rules_comment"], ACMG_assignment_comment
        )
        if ACMG_flags:
            if variants["ACMG_rule_flags"] == "-":
                ...
                variants["ACMG_rule_flags"] = update_assignment(
                    variants["ACMG_rule_flags"], ACMG_flags
                )
    ...
    def refine_pvs1_rule(self, variants):
        ...
        RefineLoF = RefineLossOfFunction(
            self.data_path,
            self.clinvar_path,
            self.clinvar_file,
            self.clinvar_stars_df,
            self.min_review_stars,
            self.beds_root,
            self.critical_prot_regions_file,
            self.clinical_exons_file,
            self.pheno_high_af_variants,
            self.phenotype_exons_file,
            self.uniprot_domains_file,
            self.hugo_genes_df,
        )
        ...
        for var_idx, var_row in variants.iterrows():
            if is_var_type4PVS1(str(var_row["variant_type"])):
                ...
                ACMG_rules, ACMG_rules_comment, var_coding_seq = RefineLoF.run(
                    self.current_sample_name,
                    str(var_row["chr"]),
                    int(var_row["start"]),
                    int(var_row["end"]),
                ...
                ACMG_rules, ACMG_rules_comment = (
                    "PVS1: NA",
                    "PVS1: not applicable for this variant type",
                )
                var_coding_seq = None
            variants.at[var_idx, "ACMG_rules"] = update_assignment(
                var_row["ACMG_rules"], ACMG_rules
            )
            variants.at[var_idx, "ACMG_rules_comment"] = update_assignment(
                var_row["ACMG_rules_comment"], ACMG_rules_comment
            )
            ...
            if current_processed_percentage - logged_processed_percentage >= 25.0:
                ...
                self.logger.info(
                    "Thread id: {}, {}, Refine PVS1 progress: {:.2f}\n ".format(
                        get_ident(),
                        ClassifyVariants.var_pos2string(var_row),
                        logged_processed_percentage,
                    )
                )
    ...
    def examine_pm4(self, variants):
        ...
        for var_idx, var_row in variants.iterrows():
            if extract_rule_result(var_row["ACMG_rules"], "PVS1") == "False" and (
                "inframe" in str(var_row["variant_type"])
                or "stop_lost" in str(var_row["variant_type"])
            ):
                ACMG_rules, ACMG_rules_comment = assign_PM4.run(
                    self.current_sample_name,
                    str(var_row["chr"]),
                    int(var_row["start"]),
                    int(var_row["end"]),
                ...
                ACMG_rules, ACMG_rules_comment = (
                    "PM4: NA",
                    "PM4: not applicable for this variant type",
                )
            variants.at[var_idx, "ACMG_rules"] = update_assignment(
                var_row["ACMG_rules"], ACMG_rules
            )
    ...
    def examine_bp3(self, variants):
        ...
        assign_BP3 = AssignBP3(
            self.data_path, self.beds_root, self.repeats_no_domains_file
        )
        ...
        for var_idx, var_row in variants.iterrows():
            if "inframe" in str(var_row["variant_type"]) or (
                extract_rule_result(var_row["ACMG_rules"], "PVS1") == "FAIL"
                and "stop_gained" in str(var_row["variant_type"])
            ):
                ACMG_rules, ACMG_rules_comment = assign_BP3.run(
                    self.current_sample_name,
                    str(var_row["chr"]),
                    int(var_row["start"]),
                    int(var_row["end"]),
                ...
                ACMG_rules, ACMG_rules_comment = (
                    "BP3: NA",
                    "BP3: not applicable for this variant type",
                )
            variants.at[var_idx, "ACMG_rules"] = update_assignment(
                var_row["ACMG_rules"], ACMG_rules
            )
    ...
    def examine_pm1(self, variants):
        ...
        assign_PM1 = AssignPM1(
            self.beds_root,
            self.critical_prot_regions_no_benign_file,
            self.pm1_regions_file,
        )
        ...
        for var_idx, var_row in variants.iterrows():
            if str(var_row["variant_type"]) == "missense":
                ACMG_rules, ACMG_rules_comment = assign_PM1.run(
                    self.current_sample_name,
                    str(var_row["chr"]),
                    int(var_row["start"]),
                    int(var_row["end"]),
                ...
                ACMG_rules, ACMG_rules_comment = (
                    "PM1: NA",
                    "PM1: not applicable for this variant type",
                )
            variants.at[var_idx, "ACMG_rules"] = update_assignment(
                var_row["ACMG_rules"], ACMG_rules
            )
    ...
    def process_clinvar_rules(self, variants):
        ...
        """
        self.logger.info(
            "Process ClinVar rules with process id: {}\n".format(get_ident())
        )
        processClinVar = ProcessClinVar(
            self.data_path,
            self.clinvar_path,
            self.clinvar_file,
            self.clinvar_stars_df,
        ...
        for var_idx, var_row in variants.iterrows():
            if "missense" in str(var_row["variant_type"]):
                ...
                ACMG_rules, ACMG_rules_comment = processClinVar.run(
                    self.current_sample_name,
                    str(var_row["chr"]),
                    int(var_row["start"]),
                    int(var_row["end"]),
                ...
                aggregated_PS1_PM5_comment = update_assignment(
                    "PS1: not applicable for this variant type",
                    "PM5: not applicable for this variant type",
                )
                variants.at[var_idx, "ACMG_rules"] = update_assignment(
                    var_row["ACMG_rules"], aggregated_PS1_PM5_class
                )
                variants.at[var_idx, "ACMG_rules_comment"] = update_assignment(
                    var_row["ACMG_rules_comment"], aggregated_PS1_PM5_comment
                )
            processed_var_counter = processed_var_counter + 1
            current_processed_percentage = (processed_var_counter / variants_num) * 100
            if current_processed_percentage - logged_processed_percentage >= 25.0:
        ...
        with open(join(self.out_path, unique_type_filename), "w") as unique_type_file:
            for unique_type in unique(variants_df["variant_type"]):
                ...
                for _, coding_and_splicing in variants_df.loc[
                    variants_df["variant_type"] == unique_type, "coding_and_splicing"
                ].iteritems():
    ...
    def filter_hl_genes(self, variants_df):
        ...
        if self.classify_only_hl_genes:
            ...
            self.logger.info(
                "Number of filtered variants affecting known HL genes: {}".format(
                    filtered_variants_df.shape[0]
                )
            )
    ...
    def mark_hl_genes(self, variant_row):
        ...
        if gene not in self.genes_inheritance_df.index:
            variant_row[
                "ACMG_rule_flags"
            ] = "Warning: Gene not in HL genes list, provided by ClinGen"
    ...
    def mark_mt_genes(self, variant_row):
        ...
        if chromosome_name == "chrMT":
            if variant_row["ACMG_rule_flags"] == "-":
                variant_row[
                    "ACMG_rule_flags"
                ] = "Warning: ACMG guidelines for HL may not be applicable for mitochondrial variants"
            else:
                variant_row["ACMG_rule_flags"] = update_assignment(
                    variant_row["ACMG_rule_flags"],
    ...
    def examine_acmg_rules(self, variants_df):
        ...
        time_end = timer()
        self.logger.info(
            "PM2, BA1, BS1, PM2_Supporting and BS1_Supporting (if applicable): {}".format(
        ...
        time_end = timer()
        self.logger.info(
            "PS1 and PM5: Elapsed time: {}".format(
                sec2hour_min_sec(time_end - time_start)
            )
        )
        ...
        ### ### ### ### ###
        self.logger.info(
            "Rule 6. Very strong pathogenic, refine PVS1 rule based on Tayoun et al. Hum. Mutat. 2018"
        )
        time_start = timer()
        variants_df = self.refine_pvs1_rule(variants_df)
        time_end = timer()
        self.logger.info(
            "PVS1: Elapsed time: {}".format(sec2hour_min_sec(time_end - time_start))
        )
        ...
        time_end = timer()
        self.logger.info(
            "PM1: Elapsed time: {}".format(sec2hour_min_sec(time_end - time_start))
        )
        ...
        time_end = timer()
        self.logger.info(
            "PM4: Elapsed time: {}".format(sec2hour_min_sec(time_end - time_start))
        )
        ...
        time_end = timer()
        self.logger.info(
            "BP3: Elapsed time: {}".format(sec2hour_min_sec(time_end - time_start))
        )
    ...
    def acmg_classify(self, variants):
        ...
        assignACMG = AssignACMG(
            variant_info, odds_pathogenicity_very_strong, x, pathogenicity_prior
        )
        preferred_inheritance, alternative_inheritance = assignACMG.prepare4classify(
            variants["ACMG_rules"], variants["ACMG_rules_comment"]
        )
        ...
        ### ### #### #### ### ###
        (
            variants["ACMG_class_preferred"],
            variants["ACMG_class_numeric_preferred"],
        ) = assignACMG.compute_categorical("preferred", preferred_inheritance)
        variants["ACMG_class_prob_preferred"] = "{:1.5f}".format(
            assignACMG.compute_probability("preferred", preferred_inheritance)
        )
        if alternative_inheritance:
            (
                variants["ACMG_class_alternative"],
                variants["ACMG_class_numeric_alternative"],
            ) = assignACMG.compute_categorical("alternative", alternative_inheritance)
            ...
            (
                variants["ACMG_class_alternative"],
                variants["ACMG_class_numeric_alternative"],
                variants["ACMG_class_prob_alternative"],
            ) = ("NA", "NA", "NaN")
        return variants
    ...
    def select_bp3_variant_rows(self, variants):
        ...
        if "inframe" in str(variants["variant_type"]) or (
            extract_rule_result(variants["ACMG_rules"], "PVS1") == "False"
            and "stop_gained" in str(variants["variant_type"])
        ):
            return True
        else:
            return False
    ...
    def select_pm4_variant_rows(self, variants):
        ...
        if extract_rule_result(variants["ACMG_rules"], "PVS1") == "False" and (
            "inframe" in str(variants["variant_type"])
            or "stop_lost" in str(variants["variant_type"])
        ):
            return True
        else:
            return False

    def filter_pathogenic_variants(
        self, classified_variant_row, pathogenic_prob_threshold
    ):
        """
        ...
        ### ### ### ###
        if (
            classified_variant_row["ACMG_class_numeric_preferred"] == 4
            or classified_variant_row["ACMG_class_numeric_preferred"] == 5
        ):
            return True
        elif (
            classified_variant_row["ACMG_class_numeric_alternative"] == 4
            or classified_variant_row["ACMG_class_numeric_alternative"] == 5
        ):
            return True
        elif (
            classified_variant_row["ACMG_class_numeric_preferred"] == 3
            and classified_variant_row["ACMG_class_prob_preferred"]
            >= pathogenic_prob_threshold
        ):
            return True
        elif (
            classified_variant_row["ACMG_class_numeric_alternative"] == 3
            and classified_variant_row["ACMG_class_prob_alternative"]
            >= pathogenic_prob_threshold
        ):
            return True
        else:
    ...
    def summarize_classification(self, classified_variants_gsvar_file):
        ...
        """
        self.logger.info(
            "Summarize classification by first loading all classified variants"
        )
        classified_variants_df, is_gsvar_empty = self.load_gsvar_df(
            "output", classified_variants_gsvar_file
        )
        if not is_gsvar_empty and self.min_pathogenicity != -1:
            ...
            filtered_pathogenic_variants_df = classified_variants_df[
                classified_variants_df.apply(
                    self.filter_pathogenic_variants,
                    pathogenic_prob_threshold=self.min_pathogenicity,
                    axis=1,
                )
            ]
            self.logger.info(
                "Number of extracted pathogenic variant(s): {}".format(
                    filtered_pathogenic_variants_df.shape[0]
                )
            )
            if filtered_pathogenic_variants_df.shape[0] > 0:
                filtered_pathogenic_variants_filename = (
                    classified_variants_gsvar_file.split("_genotoscope.GSvar")[0]
                    + "_pathogenic_vars_genotoscope.GSvar"
                )
                filtered_pathogenic_variants_df.to_csv(
                ...
                self.logger.info(
                    "Saving extracted pathogenic variants in %s",
                    join(self.out_path, filtered_pathogenic_variants_filename),
                )
    ...
    def assign_variants_class(self, gsvar_file_name, is_single_file_analysis=False):
        ...
        updated_gsvar_file_name = self.process_column_line(
            gsvar_file_name, is_single_file_analysis
        )
        ...
        if not is_gsvar_empty:
            ...
            if filtered_gsvar_df.shape[0] != 0:
                ...
                ### ### ###
                self.logger.info(
                    "ACMG class: Apply ACMG recommendations to combine met criteria and classify variants"
                )
                classified_gsvar_df = examined_gsvar_df.apply(
                    self.acmg_classify, axis=1
                )

                # preserve the initial order of the columns of the input GSvar file
                ordered_columns = self.current_gsvar_columns + [
                    "ACMG_rules",
                    "ACMG_rules_comment",
                    "ACMG_rule_flags",
                    "ACMG_class_preferred",
                    "ACMG_class_numeric_preferred",
                    "ACMG_class_prob_preferred",
                    "ACMG_class_alternative",
                ...
                # save csv file
                gsvar_out_filename = (
                    basename(gsvar_file_name).split(".GSvar")[0] + "_genotoscope.GSvar"
                )
                classified_gsvar_df.to_csv(
                ...
                ### ### ###
                self.logger.info(
                    "Filtering for variants affecting HL-relate genes, resulted to no variants"
                )
                gsvar_out_filename = (
                    basename(gsvar_file_name).split(".GSvar")[0] + "_genotoscope.GSvar"
                )
                filtered_gsvar_df.to_csv(
                    join(self.out_path, gsvar_out_filename),
                    sep="\t",
                    header=True,
                    index=False,
                ...
                self.logger.info(
                    "Saving empty classification file in %s",
                    join(self.out_path, gsvar_out_filename),
                )
            ...
            self.logger.info(
                "Error - Input GSvar file was empty, saving empty GenOtoScope classification file in %s",
                join(self.out_path, gsvar_out_filename),
            )
            return None
    ...
    def run_single_sample(self):
        self.logger.info(
            "Classify variants and assign ACMG class for single GSvar file (sample)"
        )
        ...
        if is_gsvar_file_extension(self.gsvar_path):
            ...
            time_start = timer()
            output_gsvar_file = self.assign_variants_class(
                self.gsvar_path, is_single_file_analysis
            )
            time_end = timer()
            self.logger.info(
                "Elapsed time to classify GSvar: {}".format(
                    sec2hour_min_sec(time_end - time_start)
                )
            )
    ...
    def run_multiple_samples(self):
        ...
        if len(excluded_file_names) > 0:
            ...
            self.logger.info(
                "# Excluding following contents as they don't have GSvar extension:"
            )
