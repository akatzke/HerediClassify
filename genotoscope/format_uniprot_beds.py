...
def restrict_bed_columns(input_dir, filename_in, max_columns_num):
    ...
    """
    filename_out = (
        filename_in.split(".bed")[0] + "_" + str(max_columns_num) + "columns" + ".bed"
    )
    print(
        "Restrict input: {} to {} max number of columns, outputing: {}".format(
            filename_in, max_columns_num, filename_out
        )
    )
    with open(join(input_dir, filename_in), "r") as file_in, open(
        join(input_dir, filename_out), "w"
    ) as file_out:
